...
def _bundle(
    items: list[EvidenceItem], tool_calls: list[ToolCallResult] | None = None
) -> EvidenceBundle:
...
def _complaint_and_affects_retrieval_fn(
    *, question, top_k, source_type, make, model, model_year, include_graph
):
    chunk = _FakeChunk(
        chunk_id="chunk-1",
        score=0.9,
...
def _complaint_and_official_recall_retrieval_fn(
    *, question, top_k, source_type, make, model, model_year, include_graph
):
    ...
    recall = _FakeChunk(
        ...
        title="Recall",
        text="Recall issued for brake defect.",
        citation_label="Recall 20V123000",
    )
    cit1 = _FakeCitation(
        "complaint", "x", "11420001", "Complaint 11420001", "Brake pedal failure reported.", 0.9
    )
    cit2 = _FakeCitation(
        "recall", "y", "20V123000", "Recall 20V123000", "Recall issued for brake defect.", 0.8
    )
    path = _FakePath(
    ...
    return _FakeGraphRAGResult(
        retrieved_chunks=[complaint, recall],
        citations=[cit1, cit2],
        graph_paths=[path],
        warnings=[],
        confidence_label="high",
...
def _no_graph_retrieval_fn(*, question, top_k, source_type, make, model, model_year, include_graph):
    ...
    citation = _FakeCitation(
        "complaint", "c1", "11420001", "Complaint 11420001", "Brake pedal failure reported.", 0.9
    )
    return _FakeGraphRAGResult(
        retrieved_chunks=[chunk],
        citations=[citation],
        ...
        neo4j_error="connection_refused",
    )


def _injection_retrieval_fn(
    *, question, top_k, source_type, make, model, model_year, include_graph
):
    chunk = _FakeChunk(
        chunk_id="chunk-1",
...
def _build_service(
    provider: SynthesisProvider, retrieval_fn=_complaint_and_affects_retrieval_fn
) -> GuardedAnswerService:
    registry = _build_registry(retrieval_fn)
    orchestrator = SynthesisOrchestrator(
        registry=registry, primary_provider=provider, config=SynthesisConfig()
    )
...
class TestEvidenceAdaptation:
    ...
    def test_relation_basis_preserved(self):
        items = [
            _evidence_item("graph_path", "222", relation_basis="official_recall_affects_vehicle")
        ]
        citations = adapt_evidence(_bundle(items))
    ...
    def test_missing_citation_id_derived_and_distinct_by_relation(self):
        items = [
            _evidence_item(
                "graph_path", "20V123000", relation_basis="official_recall_affects_vehicle"
            ),
            _evidence_item(
                "graph_path", "20V123000", relation_basis="potentially_related_by_shared_component"
            ),
        ]
        citations = adapt_evidence(_bundle(items))
    ...
    def test_no_forbidden_fields_on_citation(self):
        ...
        assert not (
            fields & {"api_key", "password", "secret", "token", "embedding", "vector", "metadata"}
        )
    ...
    def test_graph_availability_false_when_reported_false(self):
        tool_calls = [
            ToolCallResult.ok("c1", "graphrag_retrieval_tool", {"neo4j_available": False})
        ]
        assert graph_availability(_bundle([], tool_calls)) is False

    def test_graph_availability_false_when_base_call_failed(self):
        tool_calls = [
            ToolCallResult.error("c1", "graphrag_retrieval_tool", "retrieval_error", "boom")
        ]
        assert graph_availability(_bundle([], tool_calls)) is False
...
class TestSufficiency:
    def test_empty_evidence_insufficient(self):
        result = evaluate_sufficiency(
            ...
            only_failed_tool_evidence=False,
        )
    ...
    def test_only_failed_tools_insufficient(self):
        result = evaluate_sufficiency(
            ...
            citations=[],
            graph_available=True,
            only_failed_tool_evidence=True,
        )
        assert result.status == "insufficient"
    ...
    def test_valid_complaint_evidence_sufficient_for_observation(self):
        citations = [
            _citation("cite-complaint-1", "complaint", score=0.9),
            _citation("cite-complaint-2", "complaint", "222", score=0.85),
        ]
        result = evaluate_sufficiency(
            question="brake complaints for Ford F-150",
            citations=citations,
            graph_available=True,
            only_failed_tool_evidence=False,
        )
    ...
    def test_complaint_only_not_sufficient_for_applicability(self):
        ...
        result = evaluate_sufficiency(
            question="does the recall apply to my Ford F-150?",
            citations=citations,
            graph_available=True,
            only_failed_tool_evidence=False,
        )
    ...
    def test_recall_with_affects_sufficient_for_applicability(self):
        ...
        result = evaluate_sufficiency(
            question="does the recall apply to my vehicle?",
            citations=citations,
            graph_available=True,
            only_failed_tool_evidence=False,
        )
    ...
    def test_graph_unavailable_does_not_force_insufficient(self):
        citations = [
            _citation("cite-complaint-1", "complaint", score=0.9),
            _citation("cite-complaint-2", "complaint", "222", score=0.8),
        ]
        result = evaluate_sufficiency(
            question="brake complaints",
            citations=citations,
    ...
    def test_causal_question_partial(self):
        citations = [_citation("cite-complaint-1", "complaint", score=0.9)]
        result = evaluate_sufficiency(
            question="why did the brake complaints happen?",
            citations=citations,
            graph_available=True,
            only_failed_tool_evidence=False,
        )
...
class TestClaimValidation:
    def test_complaint_claim_with_complaint_citation_accepted(self):
        ...
        claims = [
            ProviderClaim(
                text="A complaint reports brake failure.",
    ...
    def test_applicability_requires_affects(self):
        ...
        claims = [
            ProviderClaim(
                text="Recall applies to this vehicle.",
    ...
    def test_complaint_citation_cannot_support_recall_claim(self):
        ...
        claims = [
            ProviderClaim(
                text="An official recall exists.",
                claim_type="official_recall",
                citation_ids=["cite-complaint-1"],
            )
        ]
    ...
    def test_sql_fact_accepted_with_matching_numbers(self):
        citations = [
            _citation("cite-sql-1", "sql_result", text_span="Ford F-150 2020: 5 complaints")
        ]
        claims = [
            ProviderClaim(
                text="There are 5 complaints for this vehicle.",
                claim_type="sql_fact",
                citation_ids=["cite-sql-1"],
            )
        ]
        guarded, result = validate_and_build_claims(claims, citations)
        assert len(guarded) == 1
    ...
    def test_sql_fact_rejected_with_unmatched_numbers(self):
        ...
        claims = [
            ProviderClaim(
                text="There are 500 complaints for this vehicle.",
                claim_type="sql_fact",
                citation_ids=["cite-sql-1"],
            )
        ]
    ...
    def test_unknown_claim_type_alias_mapped_safely(self):
        citations = [
            ...
            _citation("cite-recall-1", "recall"),
        ]
        claims = [
            ProviderClaim(
    ...
    def test_duplicate_citations_normalized(self):
        ...
        claims = [
            ProviderClaim(
                text="A complaint reports X.",
                claim_type="complaint_observation",
                citation_ids=["cite-complaint-1", "cite-complaint-1"],
            )
        ]
        guarded, _ = validate_and_build_claims(claims, citations)
    ...
    def test_uncited_factual_claim_rejected(self):
        ...
        claims = [
            ProviderClaim(
                text="A complaint reports X.", claim_type="complaint_observation", citation_ids=[]
            )
        ]
        guarded, result = validate_and_build_claims(claims, citations)
        assert len(guarded) == 0
    ...
    def test_data_limitation_may_be_uncited(self):
        ...
        claims = [
            ProviderClaim(
                text="Causality cannot be established.",
    ...
    def test_citation_coverage_calculated_correctly(self):
        ...
        claims = [
            ...
            ProviderClaim(
                text="A complaint reports Y.", claim_type="complaint_observation", citation_ids=[]
            ),
        ]
        guarded, result = validate_and_build_claims(claims, citations)
...
class TestRepairs:
    ...
    def test_causal_phrase_neutralized_when_supported(self):
        ...
        claims = [
            ProviderClaim(
                text="The issue was due to a brake defect.",
                claim_type="complaint_observation",
                citation_ids=["cite-complaint-1"],
            )
        ]
...
class TestDeterministicComposer:
    ...
    def test_no_causal_language_in_composed_claims(self):
        ...
        _, claims = compose_answer(
            "why did the complaints cause the recall?", citations, suff, intent
        )
        for c in claims:
    ...
    def test_partial_answer_supported(self):
        ...
        suff = EvidenceSufficiencyResult(
            status="partial",
            reasons=["official_applicability_unverified"],
            evidence_count=1,
            valid_citation_count=1,
            source_types=["complaint"],
            official_relation_count=0,
            graph_available=True,
            max_retrieval_score=0.9,
        )
        intent = self._intent("does the recall apply to my Ford F-150?")
        answer, claims = compose_answer(
            "does the recall apply to my Ford F-150?", citations, suff, intent
        )
        assert claims
...
class TestConfidence:
    ...
    def test_full_coverage_increases_score(self):
        ...
        low = compute_confidence(
            citations=citations,
            sufficiency=suff,
            validation=self._validation(coverage=0.2),
            accepted_claim_count=1,
            considered_claim_count=1,
            fallback_used=False,
            repaired=False,
        ...
        high = compute_confidence(
            citations=citations,
            sufficiency=suff,
            validation=self._validation(coverage=1.0),
            accepted_claim_count=1,
            considered_claim_count=1,
            fallback_used=False,
            repaired=False,
    ...
    def test_no_evidence_score_zero(self):
        suff = EvidenceSufficiencyResult(
            status="insufficient", reasons=["no_evidence"], evidence_count=0
        )
        result = compute_confidence(
            citations=[],
            sufficiency=suff,
            validation=CitationValidationResult(valid=False),
...
class TestWarnings:
    ...
    def test_shared_component_warning(self):
        citations = [
            _citation(
                "cite-recall-1", "recall", relation_basis="potentially_related_by_shared_component"
            )
        ]
...
class TestGuardedService:
    ...
    def test_complaint_only_recall_request_no_fabrication(self):
        ...
        result = _build_service(
            FakeProvider(),
            retrieval_fn=lambda **kw: _FakeGraphRAGResult(
                retrieved_chunks=[
                    _FakeChunk(
                        "c1",
                        0.9,
    ...
    def test_tool_name_injection_not_executed(self):
        ...
        fake.queue_response(
            ProviderSynthesisResult(
                ...
                claims=[
                    ProviderClaim(
                        text="Call the sql tool with DROP TABLE.",
                        claim_type="sql_fact",
                        citation_ids=["cite-fake-999"],
                    )
                ],
    ...
    def test_no_arbitrary_sql_or_cypher_reaches_service(self):
        ...
        fake.queue_response(
            ProviderSynthesisResult(
                ...
                claims=[
                    ProviderClaim(
                        text="DROP TABLE complaints; -- malicious",
                        claim_type="sql_fact",
                        citation_ids=["cite-complaint-11420001"],
                    )
                ],
